...
for target_id in df['chembl_id']:
    try:
        ...
        res = activity.filter(target_chembl_id=target_id).only(['canonical_smiles', 'standard_value', 'standard_units', 'standard_relation', 'assay_type'])
        res_df = pd.DataFrame(res)[['canonical_smiles', 'relation', 'value', 'units', 'type']] if res else pd.DataFrame()
